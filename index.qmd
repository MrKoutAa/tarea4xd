...
areas <- 
  regiones_socio |>
  st_transform(4326)
st_crs(registro_reptiles) <- 4326
registrosORegiones <- 
  st_join(x = registro_reptiles,
          y = dplyr::select(regiones_socio, region),
          join = st_within)
...
PaletaDEspecies <- colorFactor(palette = "green3", domain = registrosORegiones$species)
